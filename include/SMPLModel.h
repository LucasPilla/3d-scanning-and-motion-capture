...
public:
	...
	// This struct is used to store the results of the applyShape function.
	// For ceres optimization we just need restJoints.
	template <typename T>
	struct ShapeResult
	{
	...
	template <typename T>
	ShapeResult<T> applyShape(const Eigen::Matrix<T, 10, 1> &shapeParams) const
	{
	...
	// This struct is used to store the results of the applyPose function.
	// For ceres optimization we just need posedJoints.
	template <typename T>
	struct PoseResult
	{
	...
	// Uses forward kinematics through the kinematic tree.
	template <typename T>
	PoseResult<T> applyPose(const Eigen::Matrix<T, 72, 1> &poseParams,
							const Eigen::Matrix<double, 24, 3> &J_rest) const
	{
		constexpr int numJoints = 24;
		PoseResult<T> result;
