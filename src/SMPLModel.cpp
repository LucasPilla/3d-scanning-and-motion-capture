...
SMPLMesh SMPLModel::computeMesh() const
{
    ...
    Eigen::Matrix<double, Eigen::Dynamic, 3> v_shaped(N, 3);
    v_shaped = templateVertices_;
    ...
    if (beta.size() > 0)
    {
        ...
        {
            std::cerr
                << "SMPLModel::getMesh - shape_blend_shapes dimension mismatch: "
                << shapeBlendShapes_.rows() << "x" << shapeBlendShapes_.cols()
                << " vs beta size " << beta.size() << std::endl;
        }
    }

    // 2. Joint regression - use templated version directly
    Eigen::Matrix<double, Eigen::Dynamic, 3> J =
        SMPLModel::computeJoints<double>(jointRegressor_, v_shaped);

    // 3. Pose blend shapes - use templated rodrigues directly
    std::vector<Eigen::Matrix3d> rotations(numJoints);
    for (int i = 0; i < numJoints; ++i)
    {
        Eigen::Vector3d r = poseParams_.segment<3>(3 * i);
        rotations[i] = SMPLModel::rodrigues<double>(r);
    }
    ...
    // 4. Forward kinematics (inlined)
    Eigen::VectorXi parents = kinematicTree_.row(0);
    std::vector<Eigen::Matrix3d> G_rot(numJoints);
    std::vector<Eigen::Vector3d> G_trans(numJoints);

    // Root joint
    G_rot[0] = rotations[0];
    G_trans[0] = J.row(0).transpose();

    // Process children
    for (int j = 1; j < numJoints; ++j)
    {
        int p = parents(j);
        Eigen::Vector3d localT = (J.row(j) - J.row(p)).transpose();
    ...
    {
        G[i].setIdentity();
        G[i].block<3, 3>(0, 0) = G_rot[i];
        G[i].block<3, 1>(0, 3) = G_trans[i];
...
Eigen::MatrixXd SMPLModel::getJointPositions() const
{
    ...
    return SMPLModel::computeJoints<double>(jointRegressor_, v_shaped);
}